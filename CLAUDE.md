...
- **Modern Rust only.** Edition 2024, `rust-version = "1.95"`. Avoid all "orange-era" patterns documented in `PLAN.md` § "Outdated patterns to avoid": no `extern crate`, no `lazy_static!` (use `std::sync::LazyLock`), no `try!`, no `Box<dyn Error>` (use `thiserror` for libs, `anyhow` for apps), no `failure`/`error-chain`, no hand-rolled GL (use `wgpu` + WGSL), no `log`+`env_logger` (use `tracing` + `tracing-subscriber`).
- **Module layout — preference, not a rule.** `foo.rs` next to `foo/` is preferred for editor-tab clarity and symmetric refactor when a single-file module grows. `foo/mod.rs` is acceptable when contributor preference outweighs the ergonomics. Pick one style per subtree and don't mix; reviewers should not block a PR on this choice alone.
- **Pinned exact deps.** Dependency versions are pinned to exact versions (e.g. `"1.1.0"`, not `"1.1"` or `"^1.1.0"`).
- **Workspace lints.** `[workspace.lints.rust] unsafe_code = "warn"` and `[workspace.lints.clippy] pedantic = "warn"`. Default to safe Rust. `unsafe` is allowed when (a) paired with a documented `# Safety` contract on the `unsafe fn`, (b) backed by an *enforced* check — runtime assertion, structural invariant proven elsewhere, or type-level proof — not reviewer trust, and (c) confined to a single `unsafe fn` per concern with `SAFETY:` comments at every call site. Allow it on the specific module with `#[allow(unsafe_code)]` rather than relaxing the workspace lint. Example: `ECS_ROADMAP.md` issue 1b's `DenseMut::get` for multi-mut query joins.
...
- **Every PR that ships, defers, or redesigns a roadmap item updates the corresponding roadmap doc.** If a PR touches a crate that has a roadmap document under `docs/` (today: `docs/ECS_ROADMAP.md`; future: per-subsystem roadmaps as they land), and the PR ships an item listed there, defers one, splits one, or invalidates a design assumption, update the roadmap in the same PR. Concretely: mark shipped items as completed (strike-through or "✅ shipped with #N"), absorbed items as "folded in" with a one-line pointer to where, and add a `⚠️ Discuss before opening an implementation issue` callout to items whose design is still up in the air. Stale roadmap entries that contradict shipped code are worse than no roadmap — they mislead the next reader. Reviewers should treat a roadmap-touching PR with an unchanged roadmap as a missing piece, same shape as a missing README update. There is no CI gate for this today; the convention is the gate.
...
- **Crates vs modules.** A new crate under `lib/` is justified by a **distinct architectural concern with its own deps and lifecycle** — windowing, logging, input, rendering, ECS, audio. Engine-wide shared ECS types that carry a `Plugin` — resources, components, and events common across crates, starting with the `Time` resource — live in **`spark-common`** (`lib/common/`). Pure foundation types with no third-party deps and no lifecycle behaviour (math, ids, pure newtypes) live as **modules inside `spark-core`**. Rule of thumb: if it would naturally implement `Plugin`, give it its own crate.
  ...
  - **Dep-graph note.** `spark-ecs` is the *deepest* crate (stdlib only); `spark-core` depends on it because `Application` embeds a `World`. This mirrors Bevy's `bevy_ecs → bevy_app` layering and keeps the cycle-free shape Cargo requires. `spark-core` does **not** re-export ECS items — any crate that touches `World`, `Res`, `ResMut`, `IntoSystem`, or `SystemParam` adds a direct `spark-ecs` dependency. ECS belongs to `spark-ecs`; `spark-core`'s surface stays focused on `Application` / `Plugin` / `EngineError` / `Stage` (the closed frame-phase enum — a core/app concept, deliberately *not* an ECS item, so it lives in `spark-core` and mirrors how `bevy_app` owns the schedule labels while `bevy_ecs` owns the schedule machinery).
- **`Application` is plugin-driven from day one.** Through M1–M3 the binary uses `spark_core::Application` — an ordered list of plugins grouped by stage. Public surface is `new`, `add_plugin<P: Plugin>(P)`, `add_system(Stage, sys)`, `run_stage(Stage)`, `set_runner`, `run()` (there is no `add_stage` — the stages are the fixed variants of the closed `Stage` enum). No `with_window` / `with_log_filter`-style methods — every subsystem is a `Plugin`. When `spark-ecs` lands in M4, `Plugin::run` gains `&mut World` access; the trait extends, doesn't break.
