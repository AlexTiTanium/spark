...
- **Separation of concerns.** Process-wide state (logging subscriber, panic hook, root error type, boot sequence) lives in `spark-core`. Per-layer events and errors live in the layer crate (`WindowError` in `spark-window`, `RenderError` in `spark-render`, …). The binary composes everything through `spark_core::Application::new().with_window(...).run()` and never installs global state from a library crate. Libraries emit `tracing::info!(...)` and return their own `XError`; the binary picks the subscriber and unifies errors via `spark_core::EngineError`.
- **Crates vs modules.** A new crate under `lib/` is justified only by a **distinct architectural layer** — windowing, rendering, input, ECS, audio. Engine-wide infrastructure (logging, errors, math, time, ids) lives as **modules inside `spark-core`** (`spark_core::log`, `spark_core::error`, …), not as standalone crates. Crate boundaries are for compile isolation and version cadence, not for naming hygiene.
- **`Application` is the pre-ECS plugin stand-in.** Through M1–M3 the binary uses `spark_core::Application` — a small boot harness with `with_window` / `with_input` / `with_log_filter` / `run` — to wire the engine together. It is **not** an ECS `App` and does not pretend to be one; it owns config and the boot sequence only. When `spark-ecs` lands in M4, `Application` gains `add_plugin`, `world`, and the schedule driver — the API grows additively, never gets ripped out.
